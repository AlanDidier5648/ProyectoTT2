...
// Función para inicializar Dijkstra y generar pasos
function initializeDijkstra(graph, startNodeId) {
    dfsSteps = []; // Almacenar los pasos para visualización

    const distances = {}; // Almacena la distancia mínima a cada nodo
    const visited = new Set(); // Almacenar nodos visitados
    const priorityQueue = []; // Cola de prioridad simulada con un array

    // Inicializa las distancias con infinito, excepto el nodo inicial
    graph.nodes.forEach(node => {
        distances[node.id] = Infinity;
    });
    distances[startNodeId] = 0; // La distancia al nodo inicial es 0
    ...
    while (priorityQueue.length > 0) {
        ...
        priorityQueue.sort((a, b) => a.distance - b.distance); // Simula una cola de prioridad
        const current = priorityQueue.shift();
        const currentNode = current.nodeId;

        if (visited.has(currentNode)) continue; // Si ya fue visitado, lo omitimos
        visited.add(currentNode);

        // Agregamos el paso para visualización
        dfsSteps.push({ nodeId: currentNode, action: "visit" });

        // Relajamos las distancias de los vecinos
        graph.links.forEach(link => {
            if (link.source === currentNode && !visited.has(link.target)) {
                const newDistance = distances[currentNode] + (link.weight || 1); // Considera el peso del enlace
                if (newDistance < distances[link.target]) {
                    distances[link.target] = newDistance; // Actualiza la distancia
                    priorityQueue.push({ nodeId: link.target, distance: newDistance });
                    dfsSteps.push({ nodeId: link.target, action: "update", distance: newDistance });
                }
            }
        });
    }
    ...
    return distances; // Para verificar resultados
}


// Función para ejecutar el paso actual de Dijkstra
function executeCurrentStep() {
    if (currentStepIndex < 0 || currentStepIndex >= dfsSteps.length) return;

    const { nodeId, action, distance } = dfsSteps[currentStepIndex];
    ...
    switch (action) {
        case "visit":
            narration.textContent = `Procesando nodo ${nodeId}.`;
            highlightNode(nodeId, "green");
            break;
        case "update":
            narration.textContent = `Actualizando la distancia al nodo ${nodeId}: ${distance}.`;
    ...
    if (currentStepIndex === dfsSteps.length - 1) {
        Swal.fire({
            ...
            text: "El algoritmo de Dijkstra ha terminado.",
            icon: "success",
...
// Evento del botón para iniciar Dijkstra
document.getElementById("start").addEventListener("click", () => {
    if (currentGraph) {
        const startNodeId = currentGraph.nodes[0].id; // Nodo fuente (puedes permitir que el usuario lo seleccione)
        initializeDijkstra(currentGraph, startNodeId); // Llama a la función de Dijkstra
        currentStepIndex = 0;
        executeCurrentStep();
    } else {
        Swal.fire({
